...
#ClusterIssuerSpec: {
	...
	acme?: {
		...
		// Enables or disables validation of the ACME server TLS
		// certificate. If true, requests to the ACME server will not
		// have their TLS certificate validated (i.e. insecure
		// connections will be allowed). Only enable this option in
		// development environments. The cert-manager system installed
		// roots will be used to verify connections to the ACME server if
			...
			http01?: {
				...
				gatewayHTTPRoute?: {
					...
					parentRefs?: [...{
						...
						// (or empty string), this refers to the local namespace of the
						// Route.
						// Support: Core
						namespace?: strings.MaxRunes(63) & strings.MinRunes(1) & {
				...
				ingress?: {
					// The ingress class to use when creating Ingress resources to
					// solve ACME challenges that use this challenge solver. Only one
					// of 'class' or 'name' may be specified.
					class?: string
					ingressTemplate?: {
					...
					// mapping between external IPs and ingress resources.
					name?: string
					...
					podTemplate?: {
						...
						// 'serviceAccountName' and 'tolerations' fields are supported
	...
	vault?: {
		...
		// PEM-encoded CA bundle (base64-encoded) used to validate Vault
		// server certificate. Only used if the Server URL is using HTTPS
		// protocol. This parameter is ignored for plain HTTP protocol
		// connection. If not set the system root certificates are used
		// to validate the TLS connection. Mutually exclusive with
		// CABundleSecretRef. If neither CABundle nor CABundleSecretRef
		// are defined, the cert-manager controller system root
		// certificates are used to validate the TLS connection.
		caBundle?: string

		// CABundleSecretRef is a reference to a Secret which contains the
		// CABundle which will be used when connecting to Vault when
		// using HTTPS. Mutually exclusive with CABundle. If neither
		// CABundleSecretRef nor CABundle are defined, the cert-manager
		// controller system root certificates are used to validate the
		// TLS connection. If no key for the Secret is specified,
		// cert-manager will default to 'ca.crt'.
		caBundleSecretRef?: {
			// The key of the entry in the Secret resource's `data` field to
			// be used. Some instances of this field may be defaulted, in
			// others it may be required.
	...
	venafi?: {
		...
		tpp?: {
			// CABundle is a PEM encoded TLS certificate to use to verify
			// connections to the TPP instance. If specified, system roots
			// will not be used and the issuing CA for the TPP instance must
			// be verifiable using the provided root. If not specified, the
			// connection will be verified using the cert-manager system root
			// certificates.
			caBundle?: string
			credentialsRef: {
				// Name of the resource being referred to. More info:
